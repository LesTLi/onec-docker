...
Выполняет слияние указанной ветки с текущей веткой, сохраняя историю слияния (то есть создаёт коммит слияния, даже если слияние можно было бы выполнить «быстро» — fast-forward).
```bash
